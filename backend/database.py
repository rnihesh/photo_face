...
from sqlalchemy import create_engine, Column, Integer, String, Float, DateTime, Boolean, ForeignKey, LargeBinary, JSON
...
class DatabaseManager:
    ...
    def add_photo(self, file_path, file_size=None, file_hash=None, width=None, height=None):
        ...
        try:
            ...
            for cluster in clusters:
                ...
                if not cluster.representative_face_id:
                    first_face = session.query(Face).filter_by(cluster_id=cluster.id).first()
                    if first_face:
    ...
    def update_scan_progress(self, directory, last_path, files_scanned, faces_detected):
        ...
        session = self.get_session()
        try:
            progress = session.query(ScanProgress).filter_by(directory=directory, completed=False).first()
            if not progress:
    ...
    def get_stats(self):
        ...
        try:
            ...
            named_clusters = session.query(Cluster).filter(Cluster.name.isnot(None), Cluster.face_count > 0).count()
